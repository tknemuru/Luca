...
namespace TKCommon.Collections
{
    ...
    public class SparseMatrix<T>
    {
        ...
        public SparseMatrix(SparseMatrixJsonIn<T> json)
        {
            this.m_SparseItem = json.SparseItem;
            this.m_Width = json.Width;
            this.m_Height = json.Height;

            var dic = from a in json.NoSparseItemDictionary
                      select a;
            this.m_ItemDictionary = dic.ToDictionary(item => item.Key, item => item.Value);

            this.m_ItemDictionaryHorizontal = new Dictionary<int, Dictionary<int, T>>();
            
            foreach (KeyValuePair<int, T> keyValue in this.m_ItemDictionary)
            {
                int y = this.DeserializeY(keyValue.Key);
                int x = this.DeserializeX(keyValue.Key);

                if (!this.m_ItemDictionaryHorizontal.ContainsKey(y))
                {
                    this.m_ItemDictionaryHorizontal.Add(y, new Dictionary<int, T>());
                }
                this.m_ItemDictionaryHorizontal[y][this.SerializeXY(y, x)] = keyValue.Value;
            }            
        }
