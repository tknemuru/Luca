...
namespace Luca.Optimization.Test
{
    ...
    /// </summary>
    [TestFixture]
    public class BiCgStabUsingSparseVectorTest
    {
        #region "定数"
        #endregion
        ...
        public void TestExecute()
        {
            ...
            BiCgStabUsingSparseVectorLogic logic = new BiCgStabUsingSparseVectorLogic();
        ...
        {
            SparseMatrix<double> A = BiCgStabUsingSparseVectorTest.GetTranspositionTestA();
            SparseVector<double> expectedX = BiCgStabUsingSparseVectorTest.GetTranspositionTestExpectedX();
            SparseVector<double> x = new SparseVector<double>(0) { 1, 1 };

            // 正方行列に変換
            SparseMatrix<double> AtA = SparseVectorUtility.ConvertSquareMatrix(A);

            // Atx(b)を求める
            SparseVector<double> Atx = SparseVectorUtility.VectorByMatrix(AtA, expectedX);
            Console.Write("Atx(");
            ...
            BiCgStabUsingSparseVectorLogic logic = new BiCgStabUsingSparseVectorLogic();
            SparseVector<double> modifydX = logic.Execute(AtA, Atx, x);
            int i = 0;
            foreach (double value in modifydX)
            {
                Assert.AreEqual(expectedX[i], value, 0.001);
