...
namespace Luca.Optimization.Logic
{
    ...
    /// インデックスオプティマイザロジック
    /// </summary>
    public class IndexOptimaizerLogic
    {
        #region "定数"
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public int GetMatrixWidth(List<int> turnList)
        {
            StringBuilder sb = new StringBuilder();
        ...
        /// <param name="x"></param>
        public void InsertScoreIndex(List<MasterIndexJson> masterList, SparseVector<double> x, int stage)
        {
            ...
            int i = 0;
            foreach (MasterIndexJson masterRow in masterList)
            {
                if (x[i] != 0D)
                {
                    StringBuilder sb = new StringBuilder();
                    ...
                    sb.AppendLine("     ( " + STR.GetSqlString(masterRow.IndexName));
                    sb.AppendLine("     , " + masterRow.NormalizeIndexNo);
            ...
            foreach (IndexExtraInformation extInfo in IndexExtraInformation.All)
            {
                ...
                sb.AppendLine("     )");
                sb.AppendLine("VALUES");
                sb.AppendLine("     ( " + STR.GetSqlString(extInfo.Name));
                sb.AppendLine("     , 0");
                sb.AppendLine("     , " + stage.ToString());
                sb.AppendLine("     , " + x[i + extInfo.Index].ToString("R"));
                sb.AppendLine("     )");
                sb.AppendLine("    ON DUPLICATE KEY UPDATE SCORE = SCORE");
                cmdList.Add(sb.ToString());
            }
