...
namespace PoAncareAnalytics
{
    public partial class AnalyticsForm : Form
    {
        ...
        private void SetPlayer()
        {
            ...
            m_PlayerList.Add(ReversiCommon.Collections.Color.White, new PlayerHuman("あなた", ReversiCommon.Collections.Color.White));
        ...
        private void UpdateDisplay()
        {
            ...
            // 現在の評価値を取得
            ScoreIndexEvaluator ev = new ScoreIndexEvaluator(true);
            this.lblInfo.Text = ev.GetEvaluate().ToString();

            // インデックスの概要を表示
            double sum = 0;
            this.txtDescription.Text = "";
            foreach (KeyValuePair<string, double> index in ev.IndexDescription)
            {
                sum += index.Value;
                this.txtDescription.Text += index.Key + "Score:" + index.Value.ToString() + System.Environment.NewLine;
            }
            this.txtDescription.Text += "Sum:" + sum.ToString();
        }
        ...
        private void RunGame()
        {
            if (!Board.IsGameEnd())
            {
                ...
                {
                    this.lblInfo.Text = m_PlayerList[TurnKeeper.NowTurnColor].Name + "はパスしました。";
